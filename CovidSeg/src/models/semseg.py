...
class SemSeg(torch.nn.Module):
    ...
    @torch.no_grad()
    def val_on_loader(self, val_loader, savedir_images=None, n_images=0, save_preds=False, savedir=None):
        self.eval()
        ...
        for i, batch in enumerate(val_loader):
            ...
            if savedir_images and i < n_images:
                ...
                self.vis_on_batch(batch, savedir_image=os.path.join(savedir_images, "%d.jpg" % i), save_preds=save_preds)
    ...
    def vis_on_batch(self, batch, savedir_image, save_preds=False):
        ...
        ax_list[0].format_coord = lambda x, y: ''
        colors_all = np.array(['black', 'slateblue', 'orange', 'red', 'blue'])
        ...
        labels = ['Lung region', 'COVID Infection region']
        for i in range(0, 2):
            plt.plot([None], [None], label=labels[i], color=colors_all[i + 1])
        ...
        # ax_list[0].set_title('CT slice of COVID-19 patient', fontsize=12)
        # ax_list[1].set_title('CT slice with Lung and COVID-19 region',  fontsize=12)
        # ax_list[2].set_title('Prediction',  fontsize=12)
        ...
        ax_list[2].legend(fontsize=12, **legend_kwargs)
        # plt.axis('off')
